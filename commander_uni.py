...
def get_data_uni(data1=data1, data2=data2, data3=data3):
    generator = dg.QAP_Generator
    gene_train = generator('train', data1, path_dataset=DATA_PB_DIR)
    gene_train.load_dataset()
    train_data1 = gene_train.data
    gene_val = generator('val', data1, path_dataset=DATA_PB_DIR)
    gene_val.load_dataset()
    val_data1 = gene_val.data
    gene_train = generator('train', data2, path_dataset=DATA_PB_DIR)
    gene_train.load_dataset()
    train_data2 = gene_train.data
    gene_val = generator('val', data2, path_dataset=DATA_PB_DIR)
    gene_val.load_dataset()
    val_data2 = gene_val.data
    gene_train = generator('train', data3, path_dataset=DATA_PB_DIR)
    gene_train.load_dataset()
    train_data3 = gene_train.data
    gene_val = generator('val', data3, path_dataset=DATA_PB_DIR)
    gene_val.load_dataset()
    val_data3 = gene_val.data
    train_data_full = train_data1 + train_data2 + train_data3
    val_data_full = val_data1 + val_data2 + val_data3
    size_seed = 0
    hard_seed = False
    gene_train = prep.preprocess(train_data_full, n_vertices=500, size_seed=size_seed, hard_seed=hard_seed)
    gene_val = prep.preprocess(val_data_full, n_vertices=500, size_seed=size_seed, hard_seed=hard_seed)
