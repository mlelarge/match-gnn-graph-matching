...
@pytest.fixture
def make_b():
    return [torch.randn(bs, f, n, n) + torch.randint(5,(bs, f, n,n))]

def test_normalize(make_b):
    for b in make_b:
        bn = normalize(b)
        assert bn.shape == b.shape
        assert torch.allclose(bn.mean(dim = (-1,-2)), torch.zeros(bs,f), atol=ATOL)
        assert torch.allclose((4*n)*bn.var(dim = (-1,-2)), torch.ones(bs,f), atol=ATOL)
...
def test_MlpBlock_Node(b=b):
    ...
    assert torch.allclose(v12, torch.ones(bs,2*f,n), atol=ATOL)
